...
export default function ManageUsers() {
    ...
    useEffect(() => {
        ...
        // Force filter to only show students
    ...
    const confirmExtension = async () => {
        ...
        targetCourse.totalSessions = (targetCourse.totalSessions || 0) + (extensionSessions || 0);
    ...
    const handleExportExcel = async () => {
        // ... (Export logic remains same, just ensuring variables are available) ...
        try {
            ...
            worksheet.columns = [
                ...
                { header: 'วิชาที่เรียน', key: 'subject', width: 25 },
                { header: 'ชื่อนักเรียน', key: 'studentName', width: 25 },
                { header: 'ชื่อผู้ปกครอง', key: 'parentName', width: 25 },
                { header: 'โรงเรียน', key: 'school', width: 30 },
                { header: 'ระดับชั้น', key: 'level', width: 20 },
                { header: 'เวลาที่สมัคร', key: 'registeredAt', width: 20 },
                { header: 'บทบาท', key: 'role', width: 15 },
                { header: 'สถานะ', key: 'status', width: 15 }, // Added status
            ];
            ...
            for (const subject of subjectsToExport) {
                ...
                const subjectUsers = users.filter(u =>
                    u.role === 'student' &&
                    (u.enrolledSubjects?.includes(subject) || u.registeredClasses?.some(c => c.className === subject))
                );
                ...
                subjectRow.values = [`วิชา: ${subject}`];
                subjectRow.font = { bold: true, size: 14, color: { argb: 'FF1F2937' } }; // Gray-800
                subjectRow.fill = {
                    type: 'pattern',
                    pattern: 'solid',
                ...
                subjectRow.alignment = { vertical: 'middle' };
                ...
                subjectUsers.forEach((user, index) => {
                    ...
                    row.values = {
                        index: index + 1,
                        name: user.displayName || user.studentName || user.username || '-',
                        subject: subject,
                        studentName: user.studentName || '-',
                        parentName: user.parentName || '-',
                        school: user.school || '-',
                        level: user.educationLevel ? (EDUCATION_LEVEL_MAP[user.educationLevel] || user.educationLevel) : '-',
                        registeredAt: new Date(user.createdAt).toLocaleDateString('th-TH'),
                        role: user.role === 'student' ? 'นักเรียน' : 'อื่นๆ',
                    ...
                    if (user.role === 'student') {
                        roleCell.font = { color: { argb: 'FF15803D' } }; // Green-700
                    }
            ...
            saveAs(blob, `รายชื่อนักเรียน_${new Date().toISOString().split('T')[0]}.xlsx`);
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                <CardTitle className="text-lg flex items-center gap-2 font-bold text-slate-800">
                                    ...
                                    จัดการข้อมูลนักเรียน (Student Management)
                                </CardTitle>
                                <CardDescription className="text-xs mt-1">
                                    จัดการข้อมูลนักเรียนทั้งหมด แก้ไขสถานะ และดูรายละเอียด
                                </CardDescription>
                            </div>
                            <Button
                                variant="outline"
                                className="gap-2 border-emerald-200 text-emerald-700 hover:bg-emerald-50 hover:text-emerald-800 rounded-none h-9 text-sm"
                                            ...
                                            <tr key={user._id} className="hover:bg-slate-50/50 transition-colors">
                                                ...
                                                <td className="px-4 py-3">
                                                    <div className="flex flex-col gap-1 items-start">
                                                        ...
                                                        {/* Check for expired courses (Date) */}
                                                        {user.registeredCourses?.some(c => c.endDate && new Date(c.endDate) < new Date()) && (
                                                            <div className="text-[10px] text-white font-bold bg-red-500 px-2 py-0.5 rounded-none shadow-sm animate-pulse">
                                                                คอร์สจบแล้ว
                                                            </div>
                                                        )}
                                                        {/* Check for quota expired (Sessions) */}
                                                        {user.registeredCourses?.some(c => (c.totalSessions || 0) > 0 && (c.usedSessions || 0) >= (c.totalSessions || 999)) && (
                        ...
                        <div className="flex flex-col h-full">
                            ...
                            <div className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between shrink-0">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    {!isEditing && (
                                        <Button variant="ghost" size="icon" onClick={() => setShowDetails(false)} className="h-9 w-9 rounded-none">
                                                    ...
                                                    <tbody className="divide-y divide-slate-100">
                                                        {(() => {
                                                            ...
                                                            return unifiedCourses.map((course, idx) => {
                                                                ...
                                                                const isQuotaExpired = course.totalSessions > 0 && (course.usedSessions || 0) >= course.totalSessions;
                                                                const isExpired = isTimeExpired || isQuotaExpired;

                                                                return (
                                                                    <tr key={idx} className={`hover:bg-slate-50/50 transition-colors ${isExpired ? 'bg-red-50/10' : ''}`}>
            ...
            < Dialog open={isExtensionDialogOpen} onOpenChange={setIsExtensionDialogOpen} >
                <DialogContent className="sm:max-w-md rounded-none" aria-describedby="extension-dialog-desc">
                    ...
                    <div className="space-y-4">
                        <div className="space-y-2">
                            <Label className="text-slate-700 font-bold">วันที่ต่อคอร์ส</Label>
                            <Input
                                type="date"
                                value={extensionDate}
                ...
                <DialogContent className="sm:max-w-xl rounded-none border-slate-200" aria-describedby="add-course-desc">
                    ...
                    <div className="grid gap-4 py-4">
                        ...
                        <div className="p-3 bg-slate-50 border border-slate-200">
                            ...
                            <div className="flex items-center gap-4">
                                <Select
                                    value={[4, 12, 24, 48].includes(newCourse.totalSessions) ? newCourse.totalSessions.toString() : "0"}
                                    onValueChange={(val) => setNewCourse({ ...newCourse, totalSessions: parseInt(val) })}
                                >
